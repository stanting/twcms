...
<div class="m">
    <div class="head">
        <dl>
            ...
            <dt style="display:inline;float:right">
                <form class="search" method="post" action="index.php?u=photo-index">
    ...
    <div class="p c2">
        <div class="cc">
            {if:empty($cms_photo_arr)}<div class="sky warning bnote"><i></i><b>暂无内容</b></div>{else}
            <table class="tb2">
                <tr class="tit">
...
//删除
    function del(id, cid) {
        twAjax.confirm("删除不可恢复，确定删除？", function () {
            twAjax.postd("index.php?u=photo-del-ajax-1", {"id": id, "cid": cid}, function (data) {
                twAjax.alert(data);
...
//批量删除
    function batch_del() {
        twAjax.confirm("删除不可恢复，确定删除？", function () {
            ...
            twAjax.postd("index.php?u=photo-batch_del-ajax-1", {"id_arr": id_arr}, function (data) {
                twAjax.alert(data);
                if (window.twData.err == 0)
                    setTimeout(function () {
                        window.location.reload();
